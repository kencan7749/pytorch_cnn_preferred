...
# main function
def generate_image(net, layer, layer_num,feature_mask,img_mean,
                   feature_weight = 1.,
                   img_size = (224, 224 ,3),
                   initial_image = None,
                   iter_n = 200,
    ...
    img_mean = img_mean
    # image norm
    noise_img = np.random.randint(0,256,(img_size[0],img_size[1],img_size[2]))
    img_norm0 = np.linalg.norm(noise_img)
    img_norm0 = img_norm0/2.
    
    # initial image
    if initial_image is None:
        initial_image = np.random.randint(0,256,(img_size[0],img_size[1],img_size[2]))
    if save_intermediate:
    ...
    for t in range(iter_n):
        ...
        # image update
        img = np.add(img, - lr * delta_img, dtype = np.float32)[0]
        grad = grad[0]
        delta_img = delta_img[0]
