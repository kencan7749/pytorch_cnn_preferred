...
def img_preprocess(img, img_mean=np.array([0.485, 0.456, 0.406],dtype=np.float),img_std = np.array([0.229,0.224,0.225],dtype=np.float)):
...
def vid_preprocess(vid, img_mean=np.float32([104, 117, 123]),img_std = np.float32([1.,1.,1.])):
    vid = vid - img_mean
    video = np.float32(np.transpose(vid, (3, 0, 1, 2)) - np.reshape(img_mean, (3, 1, 1, 1))) / np.reshape(img_std, (3, 1, 1, 1))
    return video

def img_deprocess(img, img_mean=np.array([0.485, 0.456, 0.406],dtype=np.float),img_std = np.array([0.229,0.224,0.225],dtype=np.float)):
    '''convert from Pytorch's input image layout'''
    image = img * np.array([[img_std]]).T
    return np.dstack((image + np.reshape(img_mean, (3, 1, 1)))) * 255#[:,:,::-1]

def vid_deprocess(vid, img_mean=np.float32([104, 117, 123]), img_std = np.float32([1,1,1])):
    '''convert from pytorch's input image layout'''
    video = vid * np.reshape(img_std, (3, 1, 1, 1))
    video = video + np.reshape(img_mean, (3, 1, 1,1))
    return video.transpose(1,2,3,0)

def save_video(vid, save_name,save_intermidiate_path):
    ...
    for j in range(fr):
        writer.write(vid[j].astype(np.uint8))
    writer.release()
