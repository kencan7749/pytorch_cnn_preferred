...
from utils import img_preprocess, img_deprocess, normalise_img, p_norm, TV_norm,TV_norm_sp,TV_norm_tmp, image_norm, gaussian_blur,gaussian_blur_vid, clip_extreme_pixel, clip_small_norm_pixel, clip_small_contribution_pixel,save_video, save_gif, normalise_vid, vid_preprocess,vid_deprocess,get_cnn_features, create_feature_mask
...
def generate_preferred(net, exec_code, channel=None,
    ...
    for t in range(iter_n):
        ...
        # forward
        fw = get_cnn_features(net, input, exec_code)[-1]


        feat = torch.masked_select(fw, torch.ByteTensor(feature_mask))
        #eat = torch.bool(fw, torch.ByteTensor(feature_mask))
...
# main function
def generate_preferred_tmp(net, exec_code, channel=None,
                   feature_mask = None,
                   img_mean = (0,0,0),
                   img_std = (1,1,1),
                   norm = 255,
                   ...
                   decay_start=0.001, decay_end=0.001,
                   grad_normalize=True,
                   image_jitter=True, jitter_size=32,jitter_size_z=2,
                   image_blur=True, sigma_xy_start=2.5, sigma_xy_end=0.5, sigma_t_start=0.01, sigma_t_end=0.002,
                   use_p_norm_reg=False, p=2, lamda_start=0.5, lamda_end=0.5,
                   use_TV_norm_reg=False, TVbeta1=2, TVbeta2=2, 
                   TVlamda_start_sp=0.5, TVlamda_end_sp=0.5,
                   TVlamda_start_tmp=0.5, TVlamda_end_tmp=0.5,
                   clip_extreme=False, clip_extreme_every=4, e_pct_start=1, e_pct_end=1,
                   clip_small_norm=False, clip_small_norm_every=4, n_pct_start=5., n_pct_end=5.,
                   clip_small_contribution=False, clip_small_contribution_every=4, c_pct_start=5., c_pct_end=5.,
                   disp_every=1,
    ...
    for t in range(iter_n):
        ...
        momentum = momentum_start + t * (momentum_end - momentum_start) / iter_n
        decay = decay_start + t * (decay_end - decay_start) / iter_n
        sigma_xy = sigma_xy_start + t * (sigma_xy_end - sigma_xy_start) / iter_n
        ...
        if image_jitter:
            ...
            delta_input = np.roll(np.roll(np.roll(delta_input, ox, -1), oy, -2), oz,-3)
        # create Tensor
        input = torch.Tensor(input[np.newaxis])
        input.requires_grad_()
        # forward
        ...
        # TV norm regularization
        if use_TV_norm_reg:
            TVlamda_sp = TVlamda_start_sp + t * (TVlamda_end_sp - TVlamda_start_sp) / iter_n
            if len(input_size) == 3:
                loss_r, grad_r = TV_norm(input, TVbeta1)
                loss_r = loss_r / (img_norm0 ** 2)
                grad_r = grad_r / (img_norm0 ** 2)
                ...
                # temporal
                TVlamda_tmp = TVlamda_start_tmp + t * (TVlamda_end_tmp - TVlamda_start_tmp) / iter_n
